...
- **Two scan entry points, ONE routing contract.** The RSS full scan (`runScan`) and the discovery/single-ticker scan (`runSingleTickerScan`) MUST route by catalyst direction identically: `up` → Bullish Catalyst agent, `down` → Overreaction agent (both are LONG setups, so downstream TA/confluence direction stays `'long'`). The single-ticker path was historically overreaction-only, which silently rejected every bullish/neutral discovered ticker at "Overreaction" ("the move looks rational"). `runDiscoveryScan` passes `discoveryContext` (`reason`/`catalyst`/`direction`/`expectedMovePct`) into `runSingleTickerScan` — never re-derive a thin event there. Discovery selection is conviction-pre-qualified: the prompt encodes the downstream Buffett/Lynch judge's criteria (durable franchise, quantified asymmetric mean-reversion/under-pricing case, no value traps), the model self-rates `conviction` 0-100, and `discoverTrendingTickers` draws from a larger pool (`poolSize`) and returns the top `count` ranked by conviction — so the gauntlet's budget is spent on the strongest setups, not every mover. A pre-agent **dislocation + corroboration gate** (`DISCOVERY_FLAT_MOVE_PCT` in `constants.ts`) skips flat tickers with no directional catalyst, AND rejects a discovered catalyst whose claimed direction the real price tape contradicts on BOTH the 1-day and ~5-day windows (the model's self-reported move can be hallucinated — measure it via `MarketDataService.getHistoricalPriceAtDate` and feed the agent the measured magnitude, not the claim). Agents must receive real context (market mood, regime, sector, TA, the grounded-search news body) — a bare `Event Type: X | Severity: Y` stub is what makes a thesis thin enough for the Red Team to nuke. The single-ticker path also runs **self-consistency** on its primary agent (re-samples in the uncertainty zone, aborts on directional disagreement) and `runDiscoveryScan` scans tickers with **bounded concurrency** (`DISCOVERY_SCAN_CONCURRENCY`, respecting the Gemini rate limit). NOTE: the single-ticker path still runs only a SUBSET of the full scan's downstream gauntlet — unifying them into one shared `runGauntlet` is the deferred big-ticket item in `TODO.md`.
