        ...
        <div className="bg-sentinel-900/50 rounded-xl border border-sentinel-800/50 p-5 backdrop-blur-sm">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="bg-sentinel-950/50 rounded-lg p-3 border border-sentinel-800/50">
                    <p className="text-xs text-sentinel-500 mb-0.5">Signals</p>
                    <p className="text-lg font-bold text-sentinel-100">{digest.signalsGenerated}</p>
                </div>
                <div className="bg-sentinel-950/50 rounded-lg p-3 border border-sentinel-800/50">
                    <p className="text-xs text-sentinel-500 mb-0.5">Win Rate</p>
                    <p className={`text-lg font-bold ${digest.winRate >= 50 ? 'text-emerald-400' : digest.winRate > 0 ? 'text-red-400' : 'text-sentinel-500'}`}>
                        {digest.winRate}%
                    </p>
                </div>
            </div>
            ...
            {digest.suggestions.length > 0 && (
                <div className="pt-3 border-t border-sentinel-800/50">
                    <p className="text-xs text-sentinel-500 flex items-center gap-1 mb-2">
                        <Lightbulb className="w-3 h-3" /> Insights
                    </p>
                    <ul className="space-y-1">
                        {digest.suggestions.map((s, i) => (
                            <li key={i} className="text-xs text-sentinel-300">{s}</li>
                        ))}
                    </ul>
                </div>
            )}
